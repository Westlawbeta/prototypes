...
@Component({
  selector: 'app-search-bar-v9',
  templateUrl: './search-bar.component.html',
  styleUrls: ['./search-bar.component.less'],
})
export class SearchBarComponentv9 {
  ...
  @Input() searchType: string = 'AI Deep Search'; // Default value
  ...
  toggleOptions: ToggleOption[] = this.searchType === 'Search' ? [
    {
      label: 'Search',
      textColor: 'blue',
  ...
  selectedOption: ToggleOption = this.searchType === 'Search' ? this.toggleOptions[0] : this.toggleOptions[1];
  ...
  redirectToResearch() {
    ...
    if (this.inputContent && this.inputContent.length > 0) {
      if (this.selectedOption?.label === 'AI Deep Research') {
        ...
        }else if(window.location.pathname.includes('new-research-v8')) {
          this.router.navigate([
            '/westlaw-prototype/deep-research-v8/research-confirmation',
          ]);
        }else{
          this.router.navigate([
  ...
  trayType: string = 'recent'; // Default value
  handleTrayTypeChange(type: any) {
    this.trayType = type;
  }

  showTray: boolean = false;
  toggleShowTray(event:any) {
    this.showTray = true;
  }
  ...
  ngOnInit() {
    this.renderer.listen('document', 'click', (event: Event) => {
      if(this.showTray){
        ...
        if (!target.closest('.input-tray') && !target.closest('.search-bar')) {
          this.showTray = false;
        }
      }
    });
